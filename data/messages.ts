export const messages = {
  // Hero
  hero_available_for_new_roles: 'Available for new roles',
  hero_translate_requirements:
    'I translate complex requirements into scalable, high-performance Flutter applications. Specialized in clean architecture, advanced state management, and pixel-perfect interactions.',
  hero_mobile_excellence: 'Mobile Excellence',
  hero_years_experience_num: '+6',
  hero_years_exp: 'Years Exp.',
  hero_active_users_count: '100k+',
  hero_active_users: 'Active Users',
  hero_key_strengths: 'Key Strengths',
  hero_architecture: 'Architecture',
  hero_performance: 'Performance',
  hero_uiux: 'UX/UI',
  hero_view_mob_projects: 'View Mobile Projects',
  hero_download_cv: 'Download CV',
  hero_verified: 'verified',
  hero_deployment: 'Deployment',
  hero_production_ready: 'Production Ready',

  // Navigation
  nav_name: 'John Doe',
  nav_role: 'Senior Flutter Engineer',
  nav_link_skills: 'Skills',
  nav_link_about: 'About',
  nav_link_work: 'Work',
  nav_button_resume: 'resume',

  // About
  about_me_title: 'About Me',
  about_heading: 'More than just code',
  about_description:
    "I don't just write Dart; I build products. With over 6 years of experience in mobile development, I bridge the gap between complex engineering challenges and seamless user experiences. I specialize in optimizing app performance, reducing build sizes, and setting up scalable CI/CD workflows for engineering teams.",
  about_feature_performance: 'Performance',
  about_feature_responsive: 'Responsive',
  about_feature_clean_arch: 'Clean Arch',

  // Skills
  skills_title: 'Technical Expertise',
  skills_description: 'Core technologies and tools I use to build scalable mobile applications.',
  skills_view_full_stack: 'View full tech stack',

  // Featured Work
  featured_work_title_prefix: 'Featured ',
  featured_work_title_suffix: 'Work',
  featured_work_description:
    'Engineering seamless mobile experiences with Flutter. Focused on high-performance architectures, complex state management, and scalable design systems.',
  featured_work_view_all: 'View All Projects',

  // Projects Page
  projects_title: 'All Projects',
  projects_description:
    'A curated collection of high-performance mobile applications built with Flutter. Exploring advanced state management, custom animations, and cross-platform architecture.',
  projects_view_code: 'View Code',
  projects_live_demo: 'Live Demo',
  projects_load_more: 'Load More Projects',

  // Footer
  footer_name: 'John Doe',
  footer_role_location: 'Senior Flutter Engineer based in New York.',
  footer_linkedin: 'LinkedIn',
  footer_github: 'GitHub',
  footer_copyright: '© 2024 John Doe. Built with Tailwind CSS & Flutter love.',
}
